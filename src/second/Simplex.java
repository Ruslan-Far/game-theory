...
public class Simplex {
	...
	public static double[] answer = new double[Main.matrix[0].length - 1];
	public static int[] rows = new int[Main.matrix[0].length - 1];
	...
	public static double[][] method() {
		...
		countIteration = 0;
		columns = new int[Main.matrix.length - 1];
	...
	private static double[][] genSimplexTable() {
		double[][] simplexTable = new double
				[Main.matrix.length]
				[(Main.matrix[0].length - 1) + (Main.matrix.length - 1) + 1];

		for (int i = 0; i < simplexTable.length - 1; i++) {
			for (int j = 0; j < Main.matrix[0].length - 1; j++) {
				simplexTable[i][j] = Main.matrix[i][j];
			}
			simplexTable[i][Main.matrix[0].length - 1 + i] = 1;
			simplexTable[i][simplexTable[i].length - 1] = Main.matrix[i][Main.matrix[i].length - 1];
		}
		for (int j = 0; j < Main.matrix[0].length - 1; j++) {
			if (Main.isMin)
				simplexTable[simplexTable.length - 1][j] = Main.matrix[Main.matrix.length - 1][j];
			else
				simplexTable[simplexTable.length - 1][j] = -Main.matrix[Main.matrix.length - 1][j];
		}
		return simplexTable;
	...
	private static void getAnswer(double[][] simplexTable, int[] columns) {
		...
		countForAnswer = 0;
		System.out.print("(");
		while (count != Main.matrix[0].length - 1) {
